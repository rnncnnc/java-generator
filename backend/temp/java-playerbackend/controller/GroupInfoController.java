...
import com.qinge.playerbackend.entity.pojo.GroupInfoPojo;
import com.qinge.playerbackend.service.GroupInfoService;
...
@Tag(name = "GroupInfo接口", description = "群组表")
public class GroupInfoController  {
	 ...
	 * @return
	 */
	@Operation(summary = "根据groupName更新数据", description = "根据条件更新数据")
	@PutMapping("/update/groupName/{groupName}")
	Result updateByGroupName(@PathVariable("groupName") String groupName, @RequestBody GroupInfoPojo np) {
		Query<GroupInfoPojo> query = new Query<>();
		
		GroupInfoPojo groupInfoPojo = new GroupInfoPojo();
		
		groupInfoPojo.setGroupName(groupName);
	 ...
	 * @return
	 */
	@Operation(summary = "根据status更新数据", description = "根据条件更新数据")
	@PutMapping("/update/status/{status}")
	Result updateByStatus(@PathVariable("status") Integer status, @RequestBody GroupInfoPojo np) {
		Query<GroupInfoPojo> query = new Query<>();
		
		GroupInfoPojo groupInfoPojo = new GroupInfoPojo();
		
		groupInfoPojo.setStatus(status);
